...
You can try the app by firebase app tester, just register with this [link](https://appdistribution.firebase.dev/i/4618c1f850d9a8b1) or click on image that will open into app video on [youtube](https://www.youtube.com/watch?v=obfqVsJRhaY).
...
- Get necessary files from Drive(Solvio).

- Run `flutter pub get` to load packages.

- For run debug mode run:
  ```
  flutter run
  ```
- If you want to create a new apk in release mode, run:
  ```
  flutter build apk --release
  ```
- If you want to create a new aab in release mode, run:
  ```
...
- Package name and app versions updated because of old source code flutter app, new release key created & firebase config updated. (com.mmovie.solvio 09.09.2024)
